...
namespace cowsins
{
    ...
    public class Weapon_SO : ScriptableObject
    {
        ...
        [Tooltip("While true it allows for the gun to have a secondary shot")] public bool allowSecondaryShot;
...
// Secondary Fire data
        ...
        [Tooltip("Defines how Input is processed to shoot.")]
        public ShootingMethod shootMethod2;

        [Tooltip("In order to shoot, you need to have a 100% progress value. Define how fast you want to reach 100% here.")] public float holdProgressSpeed2;

        //[Tooltip("Type of ammunation weapon will use. Set it now for future updates. ")] public AmmoStyle ammoStyle;

        [Tooltip("The way the weapon reloads.")] public ReloadingStyle reloadStyle2;

        [Tooltip("Your bullet objects")] public Bullet projectile2;

        [Tooltip(" While true, the bullet will draw a parabola. ")] public bool projectileUsesGravity2;

        [Tooltip("time to instantiate the projectile sicne you press shoot. Keep 0 if you want shooting to be instantly"), Min(0)] public float shootDelay2;

        [Tooltip("Lifetime of the bullet")] public float bulletDuration2;

        [Tooltip("Bullet velocity")] public float speed2;

        [Tooltip("Does it explode when hits a target? If so, damage will depend on the distance between the target and the hit point")] public bool explosionOnHit2;

        [Tooltip("Toggles the capacity of hurting youtself")] public bool hurtsPlayer2;

        [Tooltip("Max reach of the explosion")] public float explosionRadius2;

        [Min(0), Tooltip("Force applied to rigidbodies on explosion")] public float explosionForce2;

        public bool continuousFire2;

        [Tooltip("Time between each shot")] public float fireRate2;

        [Tooltip("Time you will have to wait for the weapon to reload")] public float reloadTime2;

        public float coolSpeed2;

        [Range(.01f, .99f)] public float cooledPercentageAfterOverheat2;
        ...
        [Range(1,8)]public int bulletsPerFire2; // Grants the possibility of making shotguns, burstguns etc. Amount of bullets spawned every shot.

        [Tooltip("How much ammo you lose per each fire ( & per each fire point )"), Min(0)]

        public int ammoCostPerFire2;

        [Tooltip("How far the bullet is able to travel")] public int bulletRange2;
        ...
        [Tooltip("How much the bullet is able to penetrate an object")] [Range(0, 10)] public float penetrationAmount2;

        [Tooltip("Damage reduction multiplier. From 0 to 1 ( 0% to 100% )")] [Range(0, 1)] public float penetrationDamageReduction2;

        [Tooltip("While true it grants the possibility of aiming the weapon")] public bool allowAim2;
        [Tooltip("While true it allows for the gun to have a secondary shot")] public bool allowSecondaryShot2;

        [Tooltip("Distance from the camera when aiming")] public float aimDistance2;

        [Tooltip("Final Rotation aiming")] public Vector3 aimingRotation2;

        [Tooltip("Velocity to change between not aiming and aiming states")] [Range(1, 50)] public float aimingSpeed2;

        [Tooltip("Camera field of view on aiming")] [Range(1, 179)] public float aimingFOV2;

        [Tooltip("If true, you will be able to reload while aiming.")] public bool allowAimingIfReloading2;

        [Tooltip("Player Movement Speed while Aiming"), Min(.1f)] public float movementSpeedWhileAiming2;

        [Tooltip("Sets a specific speed while aiming if true")] public bool setMovementSpeedWhileAiming2;

        [Tooltip("Resize crosshair on shooting")] public float crosshairResize2;

#if UNITY_EDITOR
        [Tooltip("Attach the preset for the crosshair yo want to display while using this weapon. it MUST be a CrosshairShape Preset")] public Preset crosshairPreset2;
#endif


        [Tooltip("Apply spread per shot")] public bool applyBulletSpread2;

        [Tooltip("Velocity will be decreased depending on the weight of the weapon if this is true.")] public bool applyWeight2;

        [Tooltip("How much spread is applied.")] [Range(0, 2)] public float spreadAmount2;

        [Tooltip("How much spread is applied while aiming.")] [Range(0, 2)] public float aimSpreadAmount2;

        [Range(1, 3)]
        [Tooltip("Multiplier to increase or decrease your speed while holding the weapon")] public float weightMultiplier2 = 1f;
        // Multiplier to increase or decrease your speed while holding the weapon

        [Tooltip("Apply recoil on shooting")] public bool applyRecoil2;

        public AnimationCurve recoilX2, recoilY2;

        public float xRecoilAmount2, yRecoilAmount2;

        public float recoilRelaxSpeed2;

        public bool applyDifferentRecoilOnAiming2;

        public float xRecoilAmountOnAiming2, yRecoilAmountOnAiming2;
...
// Weapon Stats
    ...
    public class Weapon_SOEditor : Editor
    {
        ...
        override public void OnInspectorGUI()
        {
            ...
            int style2 = (int)myScript.shootStyle2;
            ...
            if (currentTab >= 0 || currentTab < tabs.Length)
            {
                switch (tabs[currentTab])
                {
                    ...
                    case "Shooting":
                        ...
                        break;
                    case "ShootingSecondary":
                        EditorGUILayout.LabelField("Secondary Shooting Settings", EditorStyles.boldLabel);
                        GUILayout.Box("", new GUILayoutOption[] { GUILayout.ExpandWidth(true), GUILayout.Height(8) });
                        if (myScript.shootStyle2 == ShootStyle.Custom)
                        {
                            EditorGUILayout.Space(10f);
                            if (GUILayout.Button("Custom Shot Weapons Tutorial", GUILayout.Height(20)))
                            {
                        ...
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("shootStyle2"));
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("shootMethod2"));

                        if (myScript.shootMethod2 == ShootingMethod.HoldUntilReady || myScript.shootMethod2 == ShootingMethod.HoldAndRelease)
                        {
                            EditorGUI.indentLevel++;
                            EditorGUILayout.PropertyField(serializedObject.FindProperty("holdProgressSpeed2"));
                            EditorGUI.indentLevel--;
                        }
                        switch (style2)
                        {
                            case 0:
                                // WeaponShootingSharedVariables(myScript);
                                WeaponSecondaryShootingSharedVariables(myScript);
                                break;
                            case 1:
                                EditorGUI.indentLevel++;
                                EditorGUILayout.PrefixLabel("Attach your Projectile here", EditorStyles.helpBox);
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("projectile2"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("projectileUsesGravity2"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("speed2"));
                                if (myScript.shootDelay2 > myScript.fireRate2) myScript.shootDelay2 = myScript.fireRate2;
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("shootDelay2"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("bulletDuration2"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("hurtsPlayer2"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("explosionOnHit2"));
                                if (myScript.explosionOnHit2)
                                {
                                    EditorGUI.indentLevel++;
                                    EditorGUILayout.PropertyField(serializedObject.FindProperty("explosionRadius2"));
                                    EditorGUILayout.PropertyField(serializedObject.FindProperty("explosionForce2"));
                                    EditorGUILayout.LabelField("Check new options under 'Visuals' tab. ", EditorStyles.helpBox);
                                    EditorGUI.indentLevel--;
                                }
                                // WeaponShootingSharedVariables(myScript);
                            ...
                            case 2:
                                ...
                                {
                                    EditorGUI.indentLevel++;
                                    EditorGUILayout.PropertyField(serializedObject.FindProperty("fireRate2"));
                                    EditorGUI.indentLevel--;
                                }
                                else EditorGUILayout.LabelField("Continuous Fire will call your custom method ONCE per frame.", EditorStyles.helpBox);
        ...
        private void WeaponSecondaryShootingSharedVariables(Weapon_SO myScript)
        {
            EditorGUILayout.Space(5f);
            ...
            {
                EditorGUI.indentLevel++;
                var fireRateProperty2 = serializedObject.FindProperty("fireRate2");
                EditorGUILayout.PropertyField(fireRateProperty2);
                EditorGUI.indentLevel--;
            }

            var bulletRangeProperty2 = serializedObject.FindProperty("bulletRange2");
            EditorGUILayout.PropertyField(bulletRangeProperty2);

            var bulletsPerFireProperty2 = serializedObject.FindProperty("bulletsPerFire2");
            EditorGUILayout.PropertyField(bulletsPerFireProperty2);

            if (myScript.bulletsPerFire2 > 1)
            {
                EditorGUI.indentLevel++;
                var timeBetweenShotsProperty2 = serializedObject.FindProperty("timeBetweenShots2");
                EditorGUILayout.PropertyField(timeBetweenShotsProperty2);
                EditorGUI.indentLevel--;
            }
            EditorGUILayout.PropertyField(serializedObject.FindProperty("ammoCostPerFire2"));
            if (myScript.ammoCostPerFire2 > myScript.bulletsPerFire2) myScript.ammoCostPerFire2 = myScript.bulletsPerFire2;

            var applyBulletSpreadProperty2 = serializedObject.FindProperty("applyBulletSpread2");
            EditorGUILayout.PropertyField(applyBulletSpreadProperty2);

            using (var group = new EditorGUILayout.FadeGroupScope(Convert.ToSingle(myScript.applyBulletSpread2)))
            {

                if (group.visible == true)
                {
                    EditorGUI.indentLevel++;
                    var spreadAmountProperty2 = serializedObject.FindProperty("spreadAmount2");
                    EditorGUILayout.PropertyField(spreadAmountProperty2);
                    if (myScript.allowAim2) EditorGUILayout.PropertyField(serializedObject.FindProperty("aimSpreadAmount2"));
                    EditorGUI.indentLevel--;
                }
            }
            if (!myScript.applyBulletSpread2) myScript.spreadAmount2 = 0;

            EditorGUILayout.PropertyField(serializedObject.FindProperty("applyRecoil2"));

            if (myScript.applyRecoil2)
            {
                EditorGUI.indentLevel++;
                EditorGUILayout.PropertyField(serializedObject.FindProperty("recoilX2"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("recoilY2"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("xRecoilAmount2"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("yRecoilAmount2"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("recoilRelaxSpeed2"));
                if (myScript.allowAim2)
                {
                    EditorGUILayout.PropertyField(serializedObject.FindProperty("applyDifferentRecoilOnAiming2"));
                    if (myScript.applyDifferentRecoilOnAiming2)
                    {
                        EditorGUI.indentLevel++;
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("xRecoilAmountOnAiming2"));
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("yRecoilAmountOnAiming2"));
                        EditorGUI.indentLevel--;
                    }
                }
                EditorGUI.indentLevel--;
            ...
            if (myScript.shootStyle2 != ShootStyle.Projectile)
            {
                var penetrationAmountProperty2 = serializedObject.FindProperty("penetrationAmount2");
                EditorGUILayout.PropertyField(penetrationAmountProperty2);
                if (myScript.penetrationAmount2 != 0)
                {
                    EditorGUI.indentLevel++;
                    var penetrationDamageReductionProperty2 = serializedObject.FindProperty("penetrationDamageReduction2");
                    EditorGUILayout.PropertyField(penetrationDamageReductionProperty2);
                    EditorGUI.indentLevel--;
                }
                EditorGUI.indentLevel--;
            }
        ...
        private void WeaponStatsSharedVariables(Weapon_SO myScript)
        {
            ...
            var allowSecondaryShotProperty = serializedObject.FindProperty("allowSecondaryShot");
            EditorGUILayout.PropertyField(allowSecondaryShotProperty);
