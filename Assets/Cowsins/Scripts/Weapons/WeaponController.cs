...
namespace cowsins
{
    public class WeaponController : MonoBehaviour
    {
        ...
        public PerformShootStyle performShootStyle2;
        ...
        public void HandleSecondaryHitscanProjectileShot()
        {
            ...
            {
                canShoot = false; // since you have already shot, you will have to wait in order to being able to shoot again
                bulletsPerFire = weapon.bulletsPerFire2;
                StartCoroutine(HandleSecondaryShooting());

                // Adding a layer of realism, bullet shells get instantiated and interact with the world
                // We should  first check if we really wanna do this
                if (weapon.showBulletShells && (int)weapon.shootStyle2 != 2)
                {
                    var bulletShell2 = Instantiate(weapon.bulletGraphics, p.position, mainCamera.transform.rotation);
                    Rigidbody shellRigidbody2 = bulletShell2.GetComponent<Rigidbody>();
                    float torque = Random.Range(-15f, 15f);
                    Vector3 shellForce = mainCamera.transform.right * 5 + mainCamera.transform.up * 5;
                    shellRigidbody2.AddTorque(mainCamera.transform.right * torque, ForceMode.Impulse);
        ...
        private IEnumerator HandleSecondaryShooting()
        {
            /// Determine wether we are sending a raycast, aka hitscan weapon, we are spawning a projectile or melee attacking
            ...
            while (i < bulletsPerFire)
            {
                ...
                {
                    yield return new WaitForSeconds(weapon.shootDelay2);
                    ProjectileShot2();
                }

                yield return new WaitForSeconds(weapon.timeBetweenShots2);
                i++;
            }
            shooting2 = false;
            yield break;
        ...
        private void ProjectileShot2()
        {
            ...
            foreach (var p in firePoint)
            {
                Bullet bullet = Instantiate(weapon.projectile2, p.position, p.transform.rotation) as Bullet;

                if (weapon.explosionOnHit2) bullet.explosionVFX = weapon.explosionVFX;
                ...
                bullet.player = this.transform;
                bullet.speed = weapon.speed;
                bullet.GetComponent<Rigidbody>().isKinematic = (!weapon.projectileUsesGravity2) ? true : false;
                bullet.damage = damagePerBullet * stats.damageMultiplier;
                bullet.duration = weapon.bulletDuration;
            }
        }
